...
function OrdersAdminPage() {
    ...
    function TableRow({ _id, count, owner, total, status, products, ptime }) {
        return (
            <tr>
                <td>{_id}</td>
                <td>{owner?.name}</td>
...
function GenReport() {
    axios
      .get("http://localhost:8080/orders/reporting", {
        responseType: 'blob',
      })
      .then((res) => {
        ...
        link.href = url;
        link.setAttribute('download', `orders_report_${currentDate}.pdf`);
